...
for (band in seq(52)) {
    for(i in 1:length(b$lon)){
        ...
        cft_nir_p[i]<-sum(outdata$cft_nir[pos_index,band]) 
        cft_water_b_p[i]<-sum(outdata$cft_water_b[pos_index,band])
        cft_water_g_p[i]<-sum(outdata$cft_water_g[pos_index,band])
        yields_p[i]<-sum(outdata$yields[pos_index,band])
        yields_irrig_p[i]<-sum(outdata$yields_irrig[pos_index,band])
        pft_harvest_p[i]<-sum(outdata$pft_harvest[pos_index,band])
        pft_irrig_p[i]<-sum(outdata$pft_irrig[pos_index,band])
        cft_frac_p[i]<-sum(outdata$cft_frac[pos_index,band])
        cft_frac_2[i]<-sum(outdata$cft_frac2[pos_index,band])
    ...
    if(RAINFED) write.csv(file=paste(band, "out_table_rainfed.csv", sep="_"), out_table_r)
    if(RAINFED==F & NATEFF==T) write.csv(file=paste(band, "out_table_nateffic.csv", sep="_"), out_table_i)
    if(SURFACE) write.csv(file=paste(band, "out_table_surface.csv", sep="_"), out_table_i)
    if(UNMUL) write.csv(file=paste(band, "out_table_withoutMultPerArea.csv", sep="_"),out_table_i) #without multiplication per area
    if(MIXED) write.csv(file=paste(band, "out_table_mixed.csv", sep="_"), out_table_i)
    if(SPRINKLER) write.csv(file=paste(band, "out_table_sprinkler.csv", sep="_"), out_table_i)
    if(DRIP) write.csv(file=paste(band, "out_table_drip.csv", sep="_"), out_table_i)
    if(IMP) write.csv(file=paste(band, "out_table_imp.csv", sep="_"), out_table_i)
}
#visualise
#for(i in 1:52){
