...
read.output.harvest2<-function(filename, ncells, nbands, startyear, year,data.size=4, band="ALL", par=1, readall=NULL){
	nyears<-file.info(filename)$size/ncells/nbands/data.size
	if(nyears/as.integer(nyears)!=1) stop("nyears:",nyears, " error\n")
	if(band[1]=="ALL") band<-c(1:nbands)
	harvest<-list()
...
# 		}
# 	}else{
			seek(fn,0,origin="start")
			data<-readBin(fn,double(),ncells*nyears*nbands,size=data.size)
			data<-array(data,c(ncells, nbands, nyears))
# 		
# 	}
...
# 	}
# 	else{
		harvest$band<-1:nbands
		harvest$year<-seq(startyear,(startyear+nyears-1))
# 	}
	close(fn)
	return(harvest)
}
